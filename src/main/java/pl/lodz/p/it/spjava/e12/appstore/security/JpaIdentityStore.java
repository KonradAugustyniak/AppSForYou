...
public class JpaIdentityStore implements IdentityStore {
    ...
    public CredentialValidationResult validate(Credential credential) {
        ...
        if (credential instanceof UsernamePasswordCredential) {
            ...
            Account account = securityEndpoint.findAccountForAuthenticationConditions(
                    usernamePasswordCredential.getCaller(),
                    usernamePasswordCredential.getPasswordAsString());
            return (null != account ? new CredentialValidationResult(account.getLogin(), new HashSet<>(Arrays.asList(account.getAccountType().toString()))) : CredentialValidationResult.INVALID_RESULT);
        }
